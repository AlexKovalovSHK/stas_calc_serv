...
export class DocumentPdfService {
  ...
  private async generatePdfContent(
    ...
    drawText('Im Barm 13 - 30916 - Isernhagen', margin, y - smallLineHeight, {
      size: smallFontSize,
    });
    ...
    invoiceInfo.forEach((line) => {
      ...
      drawText(line, page.getWidth() - margin - textWidth, infoY, {
        size: smallFontSize,
      });
    ...
    // Billing address
    drawText('Rechnungsadresse', margin, y, {
      bold: true,
      size: smallFontSize,
    });
    y -= smallLineHeight;
    ...
    y -= smallLineHeight;
    drawText(`${rechnungDto.strasse} ${rechnungDto.hausNum}`, margin, y, {
      size: smallFontSize,
    });
    y -= smallLineHeight;
    drawText(`${rechnungDto.plz} ${rechnungDto.ort}`, margin, y, {
      size: smallFontSize,
    });
    ...
    y -= smallLineHeight;
    drawText(
      `${rechnungDto.objektStrasse} ${rechnungDto.objektHausNum}`,
      margin,
      y,
      { size: smallFontSize },
    );
    y -= smallLineHeight;
    drawText(`${rechnungDto.objektPlz} ${rechnungDto.objektOrt}`, margin, y, {
      size: smallFontSize,
    });
    ...
    drawText(
      'Gesamtbetrag:',
      page.getWidth() - margin - 160,
      y - cellPadding / 2,
      { bold: true },
    );
    drawText(
      `${totalAmount}€`,
      page.getWidth() - margin - 80,
      y - cellPadding / 2,
      { bold: true },
    );
    ...
    // Footer text
    drawText(
      `Gemäß §${rechnungDto.paragraph} Abs. 1 UStG enthält der ausgewiesene Betrag keine Umsatzsteuer.`,
      margin,
      y,
      { size: smallFontSize },
    );
    y -= 2 * lineHeight;
    drawText(
      'Bitte überweisen Sie den Gesamtpreis innerhalb von 7 Tagen auf das unten angegebene Bankkonto.',
      margin,
      y,
      { size: smallFontSize },
    );
